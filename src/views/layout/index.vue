  ...
  <el-container>
    <el-header>
      <el-row class="headerline">
        ...
        <el-col :span="16">
          <div class="grid-content fontstyle">新模直聘后台管理系统</div>
        </el-col>
        <el-col :span="4">
          <div class="grid-content sy">
      ...
      <el-aside width="200px">
        <el-row>
          <el-col :span="24">
            ...
            >
              <el-menu-item v-if="!item.children" v-for="(item,index) in menuList" :key="index" :index="/'/+index+/'/">
                <i class="el-icon-menu"></i>
                <span @click="handleClick(item.path,index)">{{ item.title }}</span>
              </el-menu-item>
              <el-submenu v-if="item.children" v-for="(item,index) in menuList" :key="index" :index="/'/+index+/'/">
                <template slot="title">
                  <i class="el-icon-location"></i>
                  <span>{{ item.title }}</span>
                </template>
                  <el-menu-item :index="'1'+index" v-for="(item,index) in item.children" :key="index" @click="handleClick(item.path,index)">{{item.title}}</el-menu-item>
              </el-submenu>
